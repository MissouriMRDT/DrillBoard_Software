...
float roveSci_Grove_ReadHumid(int data_pin) {
  float sensorValue = analogRead(data_pin) * 100 / 4095; // 1023 for arduino?
  return  sensorValue;
} 

/* Tiva values: calibrate for Arduino 
	Not being used, extreme low priority */
float roveSci_FC28_ReadHumid(int data_pin) {
  float percentage = -(analogRead(data_pin) - 4095) * 100 / 1850;
  if (percentage >= 100)
	return 100;
  else
    return percentage;
